...
async function getEncryptionKey(): Promise<CryptoKey> {
  ...
  if (!keyMaterial) {
    throw new Error("TOKEN_ENCRYPTION_KEY environment variable is not configured. Cannot process tokens.");
  }
  const encoder = new TextEncoder();
...
Deno.serve(async (req) => {
  ...
  try {
    // Authenticate the caller — accept either a valid user JWT or service_role
